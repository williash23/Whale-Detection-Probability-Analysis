...
data.tmp <- read.csv("C:/Users/sara.williams/Documents/UM/Analyses/DetectionProbability/UpdatedData_Mar_2015/Whales_0714_GB1stSight_DownBay.csv")
...
hist(data_trunc_outliers$distance, breaks=bins, main="First Sighting Distance - All data", xlim=c(0,10000), ylim=c(0,800),
...
setwd("C:/Users/sara.williams/Documents/UM/Analyses/DetectionProbability/UpdatedData")
save(first_obs, file="first_obs.RData")
write.csv(first_obs, file = "first_obs.csv")

###################################################################################################
#To omit observations where behavior is NA and drop that unused level of factor

newdata <- first_obs[!(is.na(first_obs$Behavior) | first_obs$Behavior==""), ]
newdata <- newdata[!(is.na(newdata$Count_) | newdata$Count=="0"), ]
...
###############################################################
#Smaller dataframe
myvars <- c("Ship", "Count_", "Vessel_Kts", "distance", "Behavior", "Visibility", "SeaState")
newdata <- newdata[myvars]

attach(newdata)

###########################################
save(newdata, file="final_data.RData")
write.csv(newdata, file = "final_data.csv")
